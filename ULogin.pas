...
var
  Login: TLogin;
  serverS, portS, usernameS, passwordS, akses, User,dinas: string;
...
begin
  ...
  begin
    ...
    Begin
      Quser.SQL.Text :=
        'SELECT nama,Phone, username,PASSWORD,LEVEL,nama_dinas,dinas FROM USER INNER JOIN dinas ON user.dinas=dinas.kode order by Nama asc';
      QueryOPD.SQL.Text := 'Select * From Dinas order by Kode Asc'
    End
    else
    Begin
      Quser.SQL.Text :=
        'SELECT nama,Phone, username,PASSWORD,LEVEL,nama_dinas,dinas FROM USER INNER JOIN dinas ON user.dinas=dinas.kode WHERE dinas.kode='
        + dinas + ' and level="OPD" order by Nama asc';
      QueryOPD.SQL.Text := 'Select * From Dinas WHERE kode=' + dinas +
        ' order by Kode Asc'
    End;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        Login.s(Sender);
        Login.Plogin.TabIndex:=0;
        Login.sedit2.SetFocus;
      end;
    end;
  end;
end;
...
Procedure TLogin.s(Sender: TObject);
var
  Config: TIniFile;
begin
  Config := TIniFile.Create(ExtractFilePath(Application.ExeName) +
...
begin
  ...
  begin
    main.Query.SQL.Text :=
      'SELECT Username,LEVEL,dinas FROM USER WHERE USERNAME="' + sEdit2.Text +
      '" AND PASSWORD="' + sEdit3.Text + '"';
    main.Query.Open;
    ...
    begin
      ...
      QuerySettings(main.Query.FieldByName('dinas').AsString,
        main.Query.FieldByName('Level').AsString);
      sEdit2.Text := '';
      sEdit3.Text := '';
      main.Page.TabIndex := 0;
      dinas:=main.Query.FieldByName('dinas').AsString;
      main.show;
    end
    else
